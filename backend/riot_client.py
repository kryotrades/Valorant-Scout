...
class LocalAuth:
    ...
    def headers(self, refresh: bool = False) -> dict[str, Any]:
        ...
        if not isinstance(ent, dict) or not all(
            ...
            raise ClientNotReadyError(f"entitlements not ready (HTTP {resp.status_code})")
    ...
    def glz_post(self, endpoint: str, json: dict[str, Any] | None = None) -> requests.Response:
        _riot_throttle()
        self.req_count += 1
        return requests.post(
            self.glz_url + endpoint, headers=self.headers(), json=json, verify=False, timeout=8
        )
    ...
    def glz_get(self, endpoint: str) -> dict[str, Any]:
        ...
        self.req_count += 1
        return self._json(
            requests.get(self.glz_url + endpoint, headers=self.headers(), verify=False, timeout=8)
        )
    ...
    def pd_get(self, endpoint: str, refresh: bool = False, retries: int = 0) -> dict[str, Any]:
        ...
        for attempt in range(retries + 1):
            ...
            self.req_count += 1
            resp = requests.get(
                self.pd_url + endpoint, headers=self.headers(refresh), verify=False, timeout=8
            )
            if resp.status_code == 429:
                try:
...
def chat_presences(auth: LocalAuth) -> list[dict[str, Any]]:
    ...
    for index, presence in enumerate(presences):
        ...
        replaced_at = index
        break
    if replaced_at is not None and replaced_at != 0:
...
class RiotClient:
    def __init__(self) -> None:
        ...
        self._valclient: Any = None

    def get_player_overview(self, puuid: str) -> dict[str, Any]:
        order = {
    ...
    def _demo_overview(self, puuid: str) -> dict[str, Any]:
    ...
    def _official_headers(self) -> dict[str, Any]:
    ...
    def _official_overview(self, puuid: str) -> dict[str, Any] | None:
        ...
        for h in history:
            md = requests.get(
                f"{base}/val/match/v1/matches/{h['matchId']}",
                headers=self._official_headers(),
                timeout=10,
            )
            if not md.ok:
    ...
    def _local_overview(self, puuid: str) -> dict[str, Any] | None:
    ...
    def _local_name(self, client: Any, puuid: str) -> str:
        try:
    ...
    def _local_rank(
        self, client: Any, puuid: str, matches: list[dict[str, Any]]
    ) -> tuple[int, int]:
        try:
    ...
    def _normalize_match(self, raw: dict[str, Any], subject: str) -> dict[str, Any] | None:
        ...
        for p in players:
            ...
            name = (
                f"{p.get('gameName')}#{p.get('tagLine')}"
                if p.get("gameName")
                else (pid or "Unknown")[:8]
            )
            teammates.append(
                {
                    "puuid": pid,
        ...
        return {
            ...
            # _latest_tier reads this off the most recent match. It was never
            # written, so every overview built from the official or local match
            # APIs reported the player as Unranked.
            "competitiveTier": subj.get("competitiveTier") or 0,
    ...
    @staticmethod
    def _latest_tier(matches: list[dict[str, Any]]) -> int:
        for m in matches:
    ...
    def _live_instalock(
        self, agent: dict[str, Any], mode: str, region: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"ok": False, "status": "error", "message": f"Instalock failed: {e}"}

    def dodge(self, dry_run: bool = True, region: str | None = None) -> dict[str, Any]:
        if dry_run:
            print("[DODGE:DRY-RUN] would quit agent select", flush=True)
            return {
                "ok": True,
                "status": "dry-run",
                "message": "DRY-RUN: would dodge agent select. Turn dry-run OFF to actually dodge.",
            }
        try:
            auth = LocalAuth(region)
            auth.headers()
            ...
            return {"ok": True, "status": "dodged", "message": "Dodged agent select."}
        except Exception as e:
    ...
    def party_state(self, region: str | None = None) -> dict[str, Any]:
    ...
    def set_queue(
        self, queue_id: str | None, dry_run: bool = True, region: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if not snap.get("available"):
                return {
                    "ok": False,
                    "message": "Not in a party — is VALORANT fully loaded into the menus?",
                }
            elig = {e["id"] for e in snap.get("eligible") or []}
            if elig and qid not in elig:
                return {"ok": False, "message": f"{label} isn't selectable right now."}
            r = auth.glz_post(f"/parties/v1/parties/{snap['partyId']}/queue", json={"queueID": qid})
            if r.status_code >= 400:
                return {"ok": False, "message": f"Riot refused the change (HTTP {r.status_code})."}
            return {
                "ok": True,
                "status": "selected",
                "queueId": qid,
    ...
    def start_queue(self, dry_run: bool = True, region: str | None = None) -> dict[str, Any]:
        if not self._party_live():
            import sample_match
        ...
        if dry_run:
            return {
                "ok": True,
                "status": "dry-run",
                "message": "DRY-RUN: would start the queue. Turn dry-run OFF to actually queue.",
            }
        try:
            auth = LocalAuth(region)
            auth.headers()
            ...
            if r.status_code >= 400:
                return {"ok": False, "message": f"Riot refused the queue (HTTP {r.status_code})."}
            return {
                "ok": True,
                "status": "queued",
                "inQueue": True,
    ...
    def stop_queue(self, dry_run: bool = True, region: str | None = None) -> dict[str, Any]:
        if not self._party_live():
            import sample_match
        ...
        if dry_run:
            return {
                "ok": True,
                "status": "dry-run",
                "message": "DRY-RUN: would cancel the queue. Turn dry-run OFF to actually cancel.",
            }
        try:
            auth = LocalAuth(region)
            auth.headers()
            ...
            if r.status_code >= 400:
                return {"ok": False, "message": f"Riot refused the cancel (HTTP {r.status_code})."}
            return {"ok": True, "status": "idle", "inQueue": False, "message": "Queue cancelled."}
        except Exception as e:
