...
LOG = scoutlog.get_logger("history")
...
def _valid_timezone(name: str | None) -> tuple[str, tzinfo]:
    ...
    try:
        ...
        LOG.warning("no tz database available (tzdata missing?); falling back to UTC")
        return "UTC", UTC
...
def _ensure_account(
    puuid: str, riot_id: str | None = None, timezone_name: str | None = None
) -> dict[str, Any]:
...
def refresh(auth: Any, timezone_name: str | None = None) -> str | None:
    try:
        auth.headers()
        puuid = auth.puuid
    except Exception:
...
def enrich(live_match: Any, puuid: str, limit: int = 20) -> int:
    ...
    with _LOCK:
        ...
        candidates = [
            p
            for p in account.get("points", [])[-limit:]
            if p.get("matchId")
            and (p.get("acs") is None or p.get("scores") is None or p.get("partySize") is None)
        ]

    def fetch(point: dict[str, Any]) -> dict[str, Any] | None:
        try:
            detail = live_match.auth.pd_get(f"/match-details/v1/matches/{point['matchId']}")
            return live_match._career_match(detail, puuid, point["matchId"])
        except Exception:
            return None
            ...
            record(
                {
                    ...
                    or next(
                        (p.get("ts") for p in candidates if p.get("matchId") == row.get("matchId")),
                        None,
                    ),
                    "map": row.get("map"),
                    "mode": row.get("mode"),
                    ...
                    "agent": row.get("agent"),
                    "agentPortrait": row.get("agentPortrait"),
                    "agentColor": row.get("agentColor"),
                    "kills": row.get("kills"),
...
def _wl(points: list[dict[str, Any]]) -> tuple[int, int]:
    return (
        sum(p.get("result") == "Victory" for p in points),
        sum(p.get("result") == "Defeat" for p in points),
    )


def _pct(points: list[dict[str, Any]], minimum: int = _MIN_BUCKET) -> float | None:
...
def _insights(
    points: list[dict[str, Any]], timezone_name: str | None = None
) -> list[dict[str, Any]]:
    _, zone = _valid_timezone(timezone_name)
    ...
    parts = [
        (name, pct, len(bucket))
        for name, bucket in by_part.items()
        if (pct := _pct(bucket)) is not None
    ]
    if len(parts) >= 2:
    ...
    if weekend_pct is not None and weekday_pct is not None and abs(weekend_pct - weekday_pct) >= 10:
        high, low, high_label, low_label, samples = (
            ...
            if weekend_pct > weekday_pct
            else (weekday_pct, weekend_pct, "weekdays", "weekends", len(weekends) + len(weekdays))
        )
        add(
            "Weekday vs weekend",
    ...
    maps = [
        (name, pct, len(bucket))
        for name, bucket in by_map.items()
        if (pct := _pct(bucket)) is not None
    ]
    if maps:
    ...
    wins = [
        p for p in rated if p["result"] == "Victory" and isinstance(p.get("delta"), (int, float))
    ]
    losses = [
        p for p in rated if p["result"] == "Defeat" and isinstance(p.get("delta"), (int, float))
    ]
    ...
    recent = [
        p
        for p in rated
        if p["ts"] >= time.time() - 7 * 86400 and isinstance(p.get("delta"), (int, float))
    ]
    if recent:
        net = sum(p["delta"] for p in recent)
        add(
            "Last 7 days",
            f"{net:+.0f} RR across {len(recent)} competitive matches.",
            "pos" if net > 0 else "neg" if net < 0 else "neutral",
            len(recent),
            "low" if len(recent) < 8 else "medium",
        )
    ...
    if hours:
        ...
        if count >= _MIN_BUCKET:
            add(
                "Most active hour",
                f"Your largest start-time sample is {hour:02d}:00 ({count} matches).",
                samples=count,
            )
    return out[:6]
...
def _summary(points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    latest = next(
        (p for p in reversed(points) if p.get("tier") is not None and p.get("rr") is not None), None
    )
    ...
    win_deltas = [
        p["delta"]
        for p in rated
        if p.get("result") == "Victory" and isinstance(p.get("delta"), (int, float))
    ]
    loss_deltas = [
        p["delta"]
        for p in rated
        if p.get("result") == "Defeat" and isinstance(p.get("delta"), (int, float))
    ]
    next_rank = None
...
def _split_rows(points: list[dict[str, Any]], key: str) -> list[dict[str, Any]]:
    ...
    for name, matches in buckets.items():
        ...
        deltas = [d for p in matches if isinstance(d := p.get("delta"), (int, float))]
        kds = [k for p in matches if isinstance(k := p.get("kd"), (int, float))]
        acs = [a for p in matches if isinstance(a := p.get("acs"), (int, float))]
        rows.append(
            {
                "name": name,
                "games": len(matches),
                ...
                "winRate": round(100 * wins / len(matches)),
                "netRr": sum(deltas),
                "avgKd": round(sum(kds) / len(kds), 2) if kds else None,
                "avgAcs": round(sum(acs) / len(acs)) if acs else None,
                "portrait": next(
                    (p.get("agentPortrait") for p in matches if p.get("agentPortrait")), None
                ),
...
def _schedule_rows(points: list[dict[str, Any]], zone: tzinfo) -> dict[str, Any]:
    weekdays: dict[str, list[Any]] = {
        day: [] for day in ("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")
    }
    dayparts: dict[str, list[Any]] = {name: [] for name, _, _ in _DAYPARTS}
    ...
    def summarize(name: str, matches: list[dict[str, Any]]) -> dict[str, Any]:
        wins = sum(p.get("result") == "Victory" for p in matches)
...
def _milestones(points: list[dict[str, Any]]) -> dict[str, Any]:
    promotions, personal = [], {}
    ...
    if rated:
        ...
        personal = {
            "bestRrMatch": best_delta.get("matchId"),
            "bestRr": best_delta.get("delta"),
            "bestAcsMatch": best_acs.get("matchId"),
            "bestAcs": best_acs.get("acs"),
            "highestTier": highest.get("tier"),
            "highestRr": highest.get("rr"),
        }
    return {"rankChanges": promotions, "personalBests": personal}


def _act_comparison(points: list[dict[str, Any]]) -> dict[str, Any] | None:
    seasons = []
    for point in points:
    ...
    return {
        "current": {"id": current, **_summary([p for p in points if p.get("seasonId") == current])},
        "previous": {
            "id": previous,
            **_summary([p for p in points if p.get("seasonId") == previous]),
        },
    }


def payload(puuid: str | None = None, timezone_name: str | None = None) -> dict[str, Any]:
